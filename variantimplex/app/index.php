...
//define('HOST', 'http://'.$_SERVER['HTTP_HOST']."/Biosfera/trunk/index.php/");
//define('HOST2', 'http://'.$_SERVER['HTTP_HOST']."/Biosfera/trunk/");
define('ROOT',dirname(dirname(__FILE__)));
define('HOST', 'http://'.$_SERVER['HTTP_HOST']."/variantimplex/index.php/");
define('HOST2', 'http://'.$_SERVER['HTTP_HOST']."/variantimplex/");
define('APP_DIR','app');
...
define('FLASH',HOST2.APP_DIR.'/view/themes/flash/');
...
if (!include(CORE_DIR.DS.'bootstrap.php')) {
	...
	$userlbl = isset($_SESSION['userlevel'])? $_SESSION['userlevel']:1;
	/*$acl = new aclController($userid, $userlbl);
	//echo $controller;
	$res = $acl->checkAccess($controller);*/
	//echo HOST; die;
	/*if(!$res){
		//echo "<script>location.href='http://localhost/Biosfera/phpinfo.php';
        //echo "<script>location.href='http://localhost/Biosfera/trunk/index.php/cms/'";
