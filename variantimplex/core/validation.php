...
class Validation
{
 ...
 /**
     * Validates whether a phone has a valid format.
     *
     * @return bool
     * @param  string $phone
     */
	function validatePhoneNumber($phnumber){
		list($phonetmp, $exttmp) = preg_replace('/[^0-9+]/', '', split("([ ]?(x|ext)+[ ]?)", $phnumber));
        if (eregi("^(.*)[0-9]{10,20}$", $phonetmp)) {
		  return true;
		}else{
			return false;
		}
	...
	/**
     * Validates whether a fax has a valid format.
     *
     * @return bool
     * @param  string $faxnumber
     */
	function validateFaxNumber($faxnumber){
		list($phonetmp, $exttmp) = preg_replace('/[^0-9+]/', '', split("([ ]?(x|ext)+[ ]?)", $faxnumber));
        if (eregi("^(.*)[0-9]{10,20}$", $phonetmp)) {
		  return true;
		}else{
			return false;
	...
	/**
     * Validates whether a url has a valid format.
     *
     * @return bool
     * @param  string $url
     */
	...
	/**
     * Validates whether a zipcode has a valid format.
     *
     * @return bool
     * @param  string $zopcode
     */
	function validateZipcode($zipcode){
	    if (eregi("^[0-9]{5,10}$", $zipcode)) {
		  return true;
		}else{
			return false;
		}
    ...
    {
        return $this->processFunction($element, 'htmlentities');
    }
    ...
    {
        return $this->trimWordFromString($string, true);
    }
    ...
    {
        return $this->trimWordFromString($string, false);
    }
    ...
    {
        return $this->processFunction($element, 'trim');
    }
    ...
    {
        return $this->processFunction($element, 'ltrim');
    }
    ...
    {
        return $this->processFunction($element, 'rtrim');
    }
    ...
    {
        return ($check_gpc && get_magic_quotes_gpc()) ? $element : $this->processFunction($element, 'addslashes');
    }
    ...
    {
        return ($check_gpc && !get_magic_quotes_gpc()) ? $element : $this->processFunction($element, 'stripslashes');
    }
    ...
    function processFunction($element, $function)
    {
        if (function_exists($function) === true) {
            if ($this->validateArray($element, false) === false) {
                return $function($element);
            } else {
                foreach ($element as $key => $val) {
                    if ($this->validateArray($element[$key], false)) {
	...
	function implodeStringInArray($array, $implodeString){
		if(empty($array) || count($array) == 0){
			...
			return implode($implodeString, $array);
		}
	}
